...
#define MAX_BUFFER_SIZE 256 //128
#define MAX_ENDPOINT_PACKET_SIZE 64
...
static const struct usb_device_descriptor dev = {
  ...
  .bDeviceSubClass = 0,
  .bDeviceProtocol = 0,
  .bMaxPacketSize0 = MAX_ENDPOINT_PACKET_SIZE,
  .idVendor = 0x0483,
  .idProduct = 0x5740,
  .bcdDevice = 0x0200,
  .iManufacturer = 1,
...
static const struct usb_endpoint_descriptor data_endp[] = {
  {
    ...
    .wMaxPacketSize = MAX_ENDPOINT_PACKET_SIZE,
    .bInterval = 1,
  },
  {
    .bLength = USB_DT_ENDPOINT_SIZE,
...
{
  (void)ep;
  char buf[MAX_ENDPOINT_PACKET_SIZE];
  int len = usbd_ep_read_packet(usbd_dev, 0x01, buf, MAX_ENDPOINT_PACKET_SIZE);
...
static void cdcacm_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
  ...
  usbd_ep_setup(usbd_dev, 0x01, USB_ENDPOINT_ATTR_BULK, MAX_ENDPOINT_PACKET_SIZE, cdcacm_data_rx_cb);
  usbd_ep_setup(usbd_dev, 0x82, USB_ENDPOINT_ATTR_BULK, MAX_ENDPOINT_PACKET_SIZE, NULL);
...
int main(void)
{
 ...
 gpio_set_mode(GPIOA, GPIO_MODE_OUTPUT_50_MHZ,GPIO_CNF_OUTPUT_PUSHPULL,GPIO12|GPIO11);
...
void simple_usb_serial_init()
{
	...
	fifo_init(&rx_fifo,rx_buffer,MAX_BUFFER_SIZE);
...
{
  return (fifo_free(&rx_fifo));
}

inline int simple_usb_serial_check_available()
{
  return (fifo_avail(&rx_fifo));
}
...
//read  <= maxlen data and return real len
int simple_usb_serial_read(uint8_t *buff,int maxlen)
{
	int datalen = 0;
	int i;

	//if( simple_usb_serial_check_connected() == 0  ) return -1;

	datalen = simple_usb_serial_check_available();
	if( datalen > maxlen ) datalen = maxlen;

	for( i=0; i< datalen; i++){
		if( 0 > fifo_get(&rx_fifo, &buff[i]) )
			break;	
	}
	return i;
}
...
int simple_usb_serial_write_block(uint8_t *buff,int len)
{
	...
	if( simple_usb_serial_check_connected() == 0  ) return 0;

	for(i=0; i< len ; i++){
		datalen = len - i;
		if( datalen > MAX_ENDPOINT_PACKET_SIZE )
			datalen = MAX_ENDPOINT_PACKET_SIZE;
